...
| `qemu/qemu-system-arm.wasm` | the emulator, wasm64/native Wasm TCG, md5 `b62121957fa0a477487223b82a2ae1bc` | [kmehltretter82/qemu](https://github.com/kmehltretter82/qemu) branch `armv4-boards`, public commit [`0bde4768d241`](https://github.com/kmehltretter82/qemu/commit/0bde4768d2411a7acb03f9e4c25727df7992420d), built by `build/build-qemu.sh` with emsdk 4.0.23. The fork carries Kohei Tokunaga's 33-commit Wasm TCG v4 series while it is under upstream review. The RiscPC subtype models the fitted SMC FDC37C665 as an 82077AA-compatible controller rather than exposing the shared core's 82078-only commands. | GPL-2.0 |
